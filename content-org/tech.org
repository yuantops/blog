...
*** 解决方法
放弃使用 ModRun ，用自定义的 ClassLoader 替代。在实现这个 ClassLoader 时，要将当前使用的 ClassLoader 设置 parent。这样依据双亲委托机制，这样就满足了可见性原则。可以参考 [[https://github.com/eclipse-vertx/vert.x/blob/master/src/main/java/io/vertx/core/impl/IsolatingClassLoader.java][IsolatingClassLoader.java]] 。
