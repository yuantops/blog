...
:EXPORT_DESCRIPTION: 一个类被不同 classloader 加载，它们将变成不同的类
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :keywords '( "java" )
:END:
...
2. 将要加载的类所在 jar 包，连同它依赖的 jar 包，按 maven repository 目录结构放置。
3. 加载类，得到 Class ~clazz~ 。
4. 使用反射，调用 ~clazz.getDeclaredConstructor("xxxType1", "xxxType2")~ ，得到构建函数。
5. 构建函数调用 ~invoke()~, 传入参数，预期得到所需要的对象。

*** 问题描述
进行到第 4 步，就

参考链接: [[https://stackoverflow.com/questions/2999824/classcastexception-when-creating-an-instance-of-a-class-using-reflection-and-cla][StackOverflow 网友回答]] 
