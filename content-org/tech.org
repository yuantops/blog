...
最近用 ~Emacs~ ~ox-hugo~ 写了几篇文章，体验比较糟，表现为总是无理由卡顿，特别是在 org 页面按下 `Ctrl-C, Ctrl-E`导出markdown时直接卡死，好几次只能强行杀掉Emacs 进程。让人无法忍受。

总所周知，Emacs 是神的编辑器，肯定不会出问题；我不爽，必定是我不会用了。于是，我鼓足勇气，想看看是哪里出了问题。

经过辛勤搜索，各种修改配置，没有一丝进步。我心中无名火腾腾，受到极大打击，陷入灰暗心境。

无奈气馁之余，开始思考自己与 ~Emacs~ 的情感纠葛，心中冒出一个终极疑问： *为什么我要用它？？* 毕竟钻研 ~Emacs~ 配置那么久，从purcell 到spacemacs，脑子里想起Emacs，潜意识里还是充满畏难情绪，还打击了自信心。

然后我想到了答案: 对我而言， ~Emacs~ 不可取代之处在于它的 `org-mode` 模式，它写博客、记笔记非常优秀，无可替代。意识到这一点，我整个人顿悟了。
